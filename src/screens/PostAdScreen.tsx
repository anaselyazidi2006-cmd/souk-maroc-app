...
export function PostAdScreen() {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.type.startsWith('image/')) {
      alert('يجب اختيار ملف صورة فقط (JPG, PNG, WEBP...)');
      e.target.value = '';
      return;
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      const row = {
        ...
        price_label: price ? `${price} MAD` : 'بالاتفاق',
        type,
  ...
  if (!user) {
    return (
      <div style={{ background: COLORS.background, minHeight: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '0 32px', gap: 14 }}>
        ...
        <button onClick={() => nav('/login')} style={{ height: 48, padding: '0 28px', background: COLORS.primary, color: '#fff', fontWeight: 800, fontSize: 14, borderRadius: RADIUS.lg, boxShadow: SHADOW.primary }}>تسجيل الدخول</button>
        <button onClick={() => nav(-1 as unknown as string)} style={{ fontSize: 13, color: COLORS.textTertiary }}>رجوع</button>
      </div>
    );
  }
          ...
          <div style={{ display: 'flex', gap: 8, overflowX: 'auto' }} className="scrollbar-hide">
            {uploadedImage && (
              <div style={{ position: 'relative', width: 80, height: 80, borderRadius: RADIUS.lg, overflow: 'hidden', flexShrink: 0, border: `3px solid ${COLORS.primary}` }}>
                <img src={uploadedImage} alt="Uploaded" loading="lazy" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                <button onClick={() => { setUploadedImage(null); setImgIdx(0); }} style={{ position: 'absolute', top: 2, right: 2, width: 20, height: 20, borderRadius: '50%', background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', border: 'none' }}>
            ...
            <div onClick={() => fileInputRef.current?.click()} style={{ width: 80, height: 80, borderRadius: RADIUS.lg, flexShrink: 0, background: uploading ? COLORS.primary100 : COLORS.cardAlt, border: `2px dashed ${COLORS.border}`, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 4, cursor: uploading ? 'wait' : 'pointer' }}>
              {uploading ? (
                <><div style={{ width: 20, height: 20, border: `2px solid ${COLORS.primary}`, borderTopColor: 'transparent', borderRadius: '50%', animation: 'spin 1s linear infinite' }} /><span style={{ fontSize: 9, color: COLORS.textTertiary }}>رفع...</span></>
              ) : (
                <><Upload size={20} style={{ color: COLORS.textTertiary }} /><span style={{ fontSize: 9, color: COLORS.textTertiary, textAlign: 'center' }}>رفع صورة</span></>
              )}
            </div>
          </div>
        </Field>
            ...
            {showCity && (
              <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, background: COLORS.card, borderRadius: RADIUS.lg, boxShadow: SHADOW.lg, border: `1px solid ${COLORS.border}`, zIndex: 100, maxHeight: 200, overflowY: 'auto' }}>
                {MOROCCAN_CITIES.filter(c => c !== 'All Cities').map(c => (
                  <button key={c} onClick={() => { setCity(c); setShowCity(false); }} style={{ width: '100%', padding: '10px 14px', textAlign: 'right', fontSize: 13, color: city === c ? COLORS.primary : COLORS.textPrimary, fontWeight: city === c ? 800 : 500, background: city === c ? COLORS.primary100 : 'transparent', borderBottom: `1px solid ${COLORS.borderLight}` }}>
                    {c}
                  </button>
                ))}
              </div>
