...
function ListingCard({ listing }: { listing: Listing }) {
  ...
  return (
    <div style={{ background: COLORS.card, borderRadius: RADIUS.xxl, overflow: 'hidden', boxShadow: SHADOW.md, border: `1px solid ${COLORS.border}`, marginBottom: 14 }}>
      ...
      {/* Image */}
      <div style={{ position: 'relative', height: 210, overflow: 'hidden', background: COLORS.cardAlt, cursor: 'pointer' }} onClick={() => navigate('listing', listing.id)}>
      ...
      {/* Title & description */}
      <div style={{ padding: '12px 14px 8px' }} onClick={() => navigate('listing', listing.id)}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', padding: '6px 14px 12px', gap: 0, borderTop: `1px solid ${COLORS.borderLight}` }}>
        ...
        <button
          ...
          style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '7px 12px 7px 6px', borderRadius: RADIUS.lg, transition: 'background 0.15s', background: liked ? '#FEF2F2' : 'transparent' }}
        >
        ...
        <button
          ...
          style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '7px 12px', borderRadius: RADIUS.lg }}
        >
        ...
        {/* Share */}
        <button style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '7px 12px', borderRadius: RADIUS.lg }}>
        ...
        <button
          onClick={() => navigate('listing', listing.id)}
          style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '7px 10px', background: COLORS.primary100, borderRadius: RADIUS.lg }}
        >
        ...
        <a
          ...
          style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '7px 10px', background: '#DCFCE7', borderRadius: RADIUS.lg, marginLeft: 6, textDecoration: 'none' }}
...
export function HomeScreen() {
  const { navigate, user, unreadCount } = useApp();
  ...
  const filtered = LISTINGS.filter(l => {
    ...
    const matchQ = !search || l.title.includes(search) || l.city.toLowerCase().includes(search.toLowerCase());
    ...
    <div style={{ background: COLORS.background, minHeight: '100%', paddingBottom: 88 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button
              ...
              style={{ width: 40, height: 40, background: COLORS.cardAlt, borderRadius: RADIUS.lg, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', border: `1px solid ${COLORS.border}` }}
            >
              <Bell size={18} style={{ color: COLORS.textSecondary }} />
              {unreadCount > 0 && (
                <span style={{ position: 'absolute', top: 7, right: 7, width: 9, height: 9, background: COLORS.error, borderRadius: 9999, border: '2px solid white' }} />
              )}
            </button>
          </div>
        </div>
        ...
        {showCityPicker && (
          <div style={{ position: 'absolute', top: '100%', left: 16, right: 16, background: COLORS.card, borderRadius: RADIUS.xl, boxShadow: SHADOW.lg, border: `1px solid ${COLORS.border}`, zIndex: 200, maxHeight: 240, overflowY: 'auto' }}>
            {MOROCCAN_CITIES.map(city => (
              <button key={city} onClick={() => { setActiveCity(city); setShowCityPicker(false); }}
                style={{ width: '100%', padding: '11px 16px', textAlign: 'right', fontSize: 13, fontWeight: activeCity === city ? 800 : 500, color: activeCity === city ? COLORS.primary : COLORS.textPrimary, background: activeCity === city ? COLORS.primary100 : 'transparent', borderBottom: `1px solid ${COLORS.borderLight}` }}>
                {city}
              </button>
            ))}
          </div>
      ...
      <div style={{ padding: '14px 16px 0' }} onClick={() => showCityPicker && setShowCityPicker(false)}>
        ...
        {filtered.length === 0 ? (
          ...
          filtered.map((l, idx) => (
            <div key={l.id}>
              {/* Insert another ad every 5 items */}
              {idx === 4 && (
      ...
      <button
        ...
        style={{
          ...
          width: 'calc(100% - 80px)', maxWidth: 350, height: 52,
          background: `linear-gradient(135deg,${COLORS.primary},${COLORS.primary700})`,
          color: '#fff', fontWeight: 900, fontSize: 15, borderRadius: RADIUS.full,
          display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10,
          boxShadow: `${SHADOW.primary}, 0 0 0 4px rgba(232,87,42,0.15)`,
          zIndex: 90, letterSpacing: '-0.01em',
