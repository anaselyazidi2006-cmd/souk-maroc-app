...
export function ProductScreen() {
  const { productId, goBack, addToCart, toggleWishlist, isWishlisted } = useApp();
    ...
    <div style={{ background: COLORS.background, minHeight: '100%', paddingBottom: 100 }}>
      ...
      <div style={{ position: 'relative', background: COLORS.cardAlt }}>
        ...
        <div style={{ position: 'absolute', top: 0, left: 0, right: 0, display: 'flex', justifyContent: 'space-between', padding: '48px 16px 0' }}>
          <button onClick={goBack} style={{ width: 38, height: 38, background: 'rgba(255,255,255,0.92)', borderRadius: RADIUS.md, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: SHADOW.sm, backdropFilter: 'blur(6px)' }}>
            <ArrowLeft size={18} style={{ color: COLORS.textPrimary }} />
          </button>
          <div style={{ display: 'flex', gap: 8 }}>
            <button style={{ width: 38, height: 38, background: 'rgba(255,255,255,0.92)', borderRadius: RADIUS.md, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: SHADOW.sm, backdropFilter: 'blur(6px)' }}>
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          {reviews.length > 2 && (
            <button
              ...
              style={{ width: '100%', padding: '10px', border: `1.5px solid ${COLORS.border}`, borderRadius: RADIUS.lg, fontSize: 13, fontWeight: 600, color: COLORS.primary, background: 'transparent' }}
            >
      ...
      <div style={{ position: 'fixed', bottom: 0, left: '50%', transform: 'translateX(-50%)', width: '100%', maxWidth: 430, background: COLORS.card, borderTop: `1px solid ${COLORS.border}`, padding: '12px 16px', display: 'flex', gap: 10, zIndex: 100, boxShadow: '0 -4px 16px rgba(0,0,0,0.06)' }}>
        <button
          ...
          style={{ width: 50, height: 50, borderRadius: RADIUS.lg, border: `1.5px solid ${wished ? COLORS.error : COLORS.border}`, background: wished ? '#FEF2F2' : COLORS.card, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}
        >
        ...
        <button
          ...
          style={{ flex: 1, height: 50, background: product.inStock ? COLORS.primary : COLORS.textTertiary, color: '#fff', fontWeight: 800, fontSize: 14, borderRadius: RADIUS.lg, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, boxShadow: product.inStock ? SHADOW.primary : 'none' }}
        >
