...
function MyListingRow({ l }: { l: Listing }) {
  ...
  return (
    <div onClick={() => navigate('listing', l.id)} style={{ display: 'flex', gap: 10, padding: '10px 0', borderBottom: `1px solid ${COLORS.borderLight}`, cursor: 'pointer' }}>
...
export function ProfileScreen() {
  const { user, wishlist, navigate, logout, myListings } = useApp();
  const allMyListings = myListings;

  const MENU = [
    { icon: Package,    label: 'طلباتي',                sub: '3 طلبات نشطة',               action: () => navigate('orders')    },
    { icon: Heart,      label: 'المحفوظة',               sub: `${wishlist.length} منتج`,    action: () => navigate('wishlist')  },
    { icon: Bell,       label: 'الإشعارات',              sub: 'التنبيهات والتذكيرات',       action: () => navigate('notifications') },
    ...
    { icon: Info,       label: 'حول SoukPro',            sub: 'قصتنا ومهمتنا',             action: () => navigate('about')     },
  ];
  ...
  return (
    <div style={{ background: COLORS.background, minHeight: '100%', paddingBottom: 88 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, padding: '14px 16px 0' }}>
        {[
          { label: 'الطلبات',   value: String(user?.orders ?? 0),         bg: '#EFF6FF', color: COLORS.info  },
          { label: 'المحفوظة',  value: String(wishlist.length),            bg: '#FEF2F2', color: COLORS.error },
          { label: 'إعلاناتي',  value: String(allMyListings.length),       bg: COLORS.primary100, color: COLORS.primary },
      ...
      <div style={{ margin: '14px 16px 0', background: COLORS.card, borderRadius: RADIUS.xl, padding: '14px 16px', boxShadow: SHADOW.sm, border: `1px solid ${COLORS.border}` }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
          ...
          <button onClick={() => navigate('post_ad')} style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 12, fontWeight: 800, color: '#fff', background: COLORS.primary, padding: '6px 12px', borderRadius: RADIUS.md, boxShadow: SHADOW.primary }}>
            <Plus size={13} /> إضافة
          </button>
        </div>
        {allMyListings.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '20px 0' }}>
            ...
            <button onClick={() => navigate('post_ad')} style={{ marginTop: 10, fontSize: 13, fontWeight: 800, color: COLORS.primary, background: COLORS.primary100, padding: '8px 16px', borderRadius: RADIUS.lg }}>
              أضف أول إعلان 🚀
            </button>
          </div>
        ) : (
          allMyListings.map(l => <MyListingRow key={l.id} l={l} />)
        )}
      </div>
